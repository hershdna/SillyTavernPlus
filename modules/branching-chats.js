...
function endNativeAutoScrollLock() {
...
function scheduleChatScrollRestore(anchor, fallbackSourceIndex = null, documentScroll = null, { onComplete = null } = {}) {
    ...
    const restore = () => {
        ...
        if (restored) {
            // Keep the observer alive for a few frames. This covers the final
            // message-formatting pass without waiting for a quiet period or
            // allowing native auto-scroll to win a race.
            stableFrames += 1;
    ...
    const begin = () => {
        ...
        if (chatElement instanceof HTMLElement && typeof MutationObserver === 'function') {
            ...
            scrollListener = restoreAfterMutation;
            chatElement.addEventListener('scroll', scrollListener, { passive: true });
            // The ceiling is only a safety net for a missing target/lifecycle
            // signal. Normal restoration is immediate and event-driven.
    ...
    // Start after the first replacement frame, but do not wait for a quiet
...
function getBranchSwipeAction(node, siblings, direction) {
    if (!node || !Array.isArray(siblings) || siblings.length === 0) return { type: 'none' };
    const index = getDepthSiblingIndex(node, siblings);
    if (direction === 'left' && siblings.length > 1) {
        return { type: 'jump', node: siblings[(index - 1 + siblings.length) % siblings.length] };
    }
    if (direction !== 'right') return { type: 'none' };
    if (index < siblings.length - 1) return { type: 'jump', node: siblings[index + 1] };
    // User turns cannot be generated by the response generator. Chub-style
    // behavior wraps the final user swipe back to the first one instead.
    if (node.role === 'user') {
        return siblings.length > 1 ? { type: 'jump', node: siblings[0] } : { type: 'none' };
    }
    // The final non-user sibling has no existing next swipe. The caller must
    // jump to its parent and ask SillyTavern to generate a new continuation.
