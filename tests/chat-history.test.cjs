...
function fixture() {
    ...
    return { api: sandbox.api, sandbox, nodes, document,
...
test('injection is idempotent and Clear removes active injection', async () => {
    ...
    assert.equal(prompt.filter(m => m.extra?.stplusChatHistoryInjection).length, 1);
    await f.api.clearCurrentSummary();
    await f.api.injectCurrentSummary(prompt);
    assert.equal(prompt.filter(m => m.extra?.stplusChatHistoryInjection).length, 0);
});
